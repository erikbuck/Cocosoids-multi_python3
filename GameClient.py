...
class ClientPlayLayerAction(cocos.actions.Action):
    ...
    def handleLocalKeyboard(self):
      ...
      if pyglet.window.key._1 in self.target.keys_being_pressed:
         self.target.cshieldPlayer()
      else:
         self.target.cunshieldPlayer()
...
class ClientPlayLayer(CommonLayers.PlayLayer):
    ...
    def cshieldPlayer(self):
      #print('shieldPlayer')
      connection.Send({"action": "shieldPlayer",
         "shieldPlayer":''})

    def cunshieldPlayer(self):
      connection.Send({"action": "unshieldPlayer",
         "unshieldPlayer":''})
